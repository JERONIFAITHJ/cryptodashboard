      ...
      {coinData.map((val, i) => {
        return (
          <motion.div
            ...
            transition={{ duration: 0.3, ease: 'easeOut', delay: (i * 0.1)%0.5 }}
            key={i}
          ...
          >
            ...
            <div className={classes.firstChild}>
              ...
              <div className={classes.headContainer}>
                <h2 onClick={() => navigateToCoinPage(val.id)}>{val.symbol.toUpperCase()}</h2>
                <h3 onClick={() => navigateToCoinPage(val.id)}>{val.name}</h3>
              </div>
              <BookmarkIcon onClick={() => bookmark(val.id)} className={classes.bookmark} />
            </div>
            ...
            <div className={classes.secondChild}>
              ...
              {val.price_change_percentage_24h > 0 ? (
                <TrendingUpRoundedIcon onClick={() => navigateToCoinPage(val.id)} className={classes.trendingUpIcon} />
              ) : (
                <TrendingDownRoundedIcon onClick={() => navigateToCoinPage(val.id)} className={classes.trendingDownIcon} />
              )}
            </div>
            ...
            <div className={classes.fourthChild}>
              <p onClick={() => navigateToCoinPage(val.id)}>{`Total volume: $ ${val.total_volume.toLocaleString()}`}</p>
              <p onClick={() => navigateToCoinPage(val.id)}>{`Total market cap: $ ${val.market_cap.toLocaleString()}`}</p>
            </div>
          </motion.div>
        );
      })}
